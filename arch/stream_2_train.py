...
def main():
    ...
    bestModelPath = "two_stream_fusion_" + params['model'] + "_" + time_str + ".hdf5"
    traincsvPath = "two_stream_train_plot_" + params['model'] + "_" + time_str + ".csv"
    valcsvPath = "two_stream_val_plot_" + params['model'] + "_" + time_str + ".csv"
    ...
    with tf.device('/gpu:0'):
        for epoch in range(params['nb_epochs']):
            ...
            for trainIDS in train_splits:
                ...
                x_train_rgb, x_train_flow, y_train_pose, y_train_object, y_train_human = load_split(trainIDS, labels_train, params['dim'], params['n_channels'], 10, rgb_dir, flow_dir, train=True)
            ...
            for valIDS in val_splits:
                x_val_rgb = x_val_flow = y_val_pose = y_val_object = y_val_human = x_train_rgb = x_train_flow = y_train_pose = y_train_object = y_train_human = None
                x_val_rgb, x_val_flow, y_val_pose, y_val_object, y_val_human = load_split(valIDS, labels_val, params['dim'], params['n_channels'], 10, rgb_dir, flow_dir, train=True)
