...
def load_set(annot_path, setlist_path, datadir, dim, n_channels):
    ...
    X = np.zeros([12000, dim[0], dim[1], n_channels])  # overshooting size. will cut to right size at the end of this function.
    y = np.zeros(12000)  # overshooting size. will cut to right size at the end of this function.
    dataIndex = 0
    for key in keys:
        if key in annots:
            dirinfo = key.split('/')
