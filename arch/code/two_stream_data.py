...
def load_split(ids, labels, dim, n_channels, of_len, rgb_dir, flow_dir, set_type, encoding, train=True, crop=True):
    ...
    for i, ID in enumerate(ids):
        ...
        if crop is True:
            ofbbs = str(float(split_id[2])) + "_" + str(float(split_id[3])) + "_" + str(float(split_id[4])) + "_" + str(float(split_id[5]))
            ofvid_name = vid_name + "_" + keyframe + "_" + ofbbs
        else:
            ofvid_name = vid_name + "_" + keyframe
        ...
        for fn in range(-of_len // 2, of_len // 2):
            ...
            if encoding == "grayscale":
                ...
                f_img_name = flow_dir + set_type + "/" + ofvid_name + "/frame" + str('{:06}'.format(of_frame)) + ".jpg"
                # print(f_img_name)
