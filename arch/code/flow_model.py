...
def flow_create_model(classes, model_name, soft_sigmoid=True, image_shape=(224, 224), opt_flow_len=20, freeze_all=True, conv_fusion=False):
    ...
    if model_name == "resnet50":
        base_model = resnet50_of(input_shape, pooling=None)  # resnet
    elif model_name == "inceptionv3":
        # NOTE Weights upon model building are imagenet but kinetics weights are loaded, see rgb.py
        base_model = InceptionV3_TSN(include_top=False, weights='imagenet', input_shape=input_shape, pooling=None)
    x = base_model.output
