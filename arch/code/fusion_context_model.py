...
def prepare_context_streamLSTM(classes, context_weights, NHU1, timewindow, n_features):
    original_context_stream = context_create_modelB(NHU1, NHU1 / 2, timewindow, n_features)
    if context_weights is None:
        print("Aborting, No saved flow_weights weights file, please use fusion weights!")
        sys.exit(1)
    else:
...
class FusionContextModelLSTM():

    def __init__(self, classes, rgb_weights, flow_weights, context_weights, NHU1, timewindow, n_features):
        # Simple non-time-distributed model
        rgb_m = prepare_rgb_stream(classes, rgb_weights, "resnet50")
        rgb_m.layers.pop()
        rgb_m.layers.pop()
        ...
        flow_m.layers.pop()
        flow_m.layers.pop()
        context_m = prepare_context_streamLSTM(classes, context_weights, NHU1, timewindow, n_features)
        context_m.layers.pop()
        context_m.layers.pop()
        context_m.layers.pop()
        # TODO Maybe not so many pops
        ...
        self.model = Model(inputs=[rgb_m.input, flow_m.input, context_m.input[0], context_m.input[1]], outputs=[pred_pose, pred_obj_human, pred_human_human])
