...
from rgbUCF_model import rgb_create_model
from flowUCF_model import flow_create_model
import sys
import utils


def prepare_rgb_stream(n_classes, rgb_weights, model_name):
    original_rgb_stream,layer_names = rgb_create_model(n_classes, model_name=model_name, freeze_all=True, conv_fusion=False)
...
def prepare_flow_stream(n_classes, flow_weights, model_name):
    original_flow_stream,layer_names = flow_create_model(n_classes, model_name=model_name, soft_sigmoid=True, image_shape=(224, 224), opt_flow_len=20, freeze_all=True, conv_fusion=False)
    ...
    if flow_weights is None:
        ...
        for layer in original_flow_stream.layers:
            ...
            layer.name = layer.name + str("_of")
            print(layer.name)
            layer.trainable = False
        original_flow_stream.layers[-1].name = original_flow_stream.layers[-1].name + '_of'
        return original_flow_stream


class TwoStreamModel():

    def __init__(self, n_classes, rgb_weights, flow_weights):
        # Simple non-time-distributed model
        rgb_m = prepare_rgb_stream(n_classes, rgb_weights, "resnet50")
        rgb_m.layers.pop()
        rgb_m.layers.pop()
        rgb_m.layers.pop()
        flow_m = prepare_flow_stream(n_classes, flow_weights, "resnet50")
        flow_m.layers.pop()
        flow_m.layers.pop()
        flow_m.layers.pop()
        ...
        self.model = Model(inputs=[rgb_m.input, flow_m.input], outputs=[pred])

        print(self.model.summary())
        print("Total number of layers in model: " + str(len(self.model.layers)))
